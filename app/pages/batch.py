...
def show():
    ...
    with col_dl2:
        st.download_button(label="Télécharger le modèle CSV", data=csv_data,
            file_name="template_etudiants.csv", mime="text/csv", width="stretch")
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.expander("Aperçu des données importées"):
                st.dataframe(df_uploaded.head(10), width="stretch", height=300)
                st.caption(f"Affichage des 10 premières lignes sur {len(df_uploaded)}")
            
            col_p1, col_p2, col_p3 = st.columns([1, 0.5, 1])
            with col_p2:
                batch_predict = st.button("Lancer les prédictions", width="stretch")
                    ...
                    try:
                        ...
                        with col_d2:
                            st.download_button(label="Télécharger les résultats (CSV)", data=result_csv,
                                file_name="predictions_decrochage.csv", mime="text/csv", width="stretch")
