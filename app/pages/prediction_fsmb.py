...
def show():
    ...
    if "fsmb_prediction_done" not in st.session_state:
        st.session_state.fsmb_prediction_done = False
    if "fsmb_form_data" not in st.session_state:
        st.session_state.fsmb_form_data = {}

    if predict_clicked:
        st.session_state.fsmb_form_data = input_values.copy()
        st.session_state.fsmb_prediction_done = True

    if st.session_state.fsmb_prediction_done and st.session_state.fsmb_form_data:
        input_dict = st.session_state.fsmb_form_data
        ...
        with st.spinner("Analyse en cours..."):
            try:
                class_idx, class_label, probas = predict_single(input_dict)
                ...
                try:
                    fig_shap = generate_shap_bar(input_dict)
